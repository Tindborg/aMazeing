...
namespace ShaderGraphEssentials
{
    ...
    {
        private const string SGEVersion = "1.1.4";
        ...
        private const string SimpleLitScene = LWRPDemoPath + "/Scenes/ShaderGraphEssentials_Showcase_SimpleLit.unity";
        private const string ToonLitScene = LWRPDemoPath + "/Scenes/ShaderGraphEssentials_Showcase_ToonLit.unity";
        private const string WaterScene = LWRPDemoPath + "/Scenes/ShaderGraphEssentials_Showcase_Water.unity";

        private const string DemoFolderPath = "DemoScenes";
        private const string BaseDemoPath = "DemoScenes/Demo_Base";
        private const string LWRPDemoPath = "DemoScenes/Demo_LWRP";

        private const string BasePluginPath = "Plugin/Editor/Plugin_Base";
        private const string LWRPPluginPath = "Plugin/Editor/Plugin_LWRP";

        private const string LWRPPluginPackagePath = "Packages/SGE_LWRP.unitypackage";
        private const string LWRPDemoPackagePath = "Packages/SGE_LWRP_Demo.unitypackage";
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(0, 0, defaultXSize, 30), "LWRP", EditorStyles.boldLabel);

            if (GUI.Button(new Rect(0, 30, defaultXSize, ButtonHeight), "Import LWRP plugin"))
            {
                ImportLWRPPlugin();
            }

            bool isLWRPImported = IsLWRPImported();

            using (new EditorGUI.DisabledScope(!isLWRPImported))
            {
                if (GUI.Button(new Rect(0, ButtonHeight * 2 + Margin, defaultXSize, ButtonHeight), "Import LWRP Demo scenes"))
                {
                    ImportLWRPScenes();
                }
            }
            
            bool areLWRPScenesImported = AreLWRPScenesImported();

            using (new EditorGUI.DisabledScope(!areLWRPScenesImported))
            {
            ...
            GUI.Label(new Rect(0, 0, defaultXSize, 30), "Switch between LWRP / HDRP", EditorStyles.boldLabel);
            
            GUI.Label(new Rect(0, 30, defaultXSize, 30), "To switch between LWRP / HDRP, hit the button below and reimport the correct plugin"
            , _wrapLabelStyle);
        ...
        private bool AreLWRPScenesImported()
        {
            return Directory.Exists(Path.Combine(SGEPath, LWRPDemoPath));
        }

        private bool IsLWRPImported()
        {
            return Directory.Exists(Path.Combine(SGEPath, LWRPPluginPath));
        }

        private void ImportLWRPScenes()
        {
            ImportPackage(LWRPDemoPackagePath);
        }

        private void ImportLWRPPlugin()
        {
            ImportPackage(LWRPPluginPackagePath);
        }
        ...
        private void CleanEverything()
        {
            ...
            DeleteIfFolderExist(LWRPPluginPath);
